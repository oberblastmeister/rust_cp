...
mod cp_library {
    pub mod io {
        ...
        impl<W: Write> Cout<W> {
            ...
            pub fn put(&mut self, value: impl Display) -> &mut Self {
        ...
        mod tests {
            ...
            fn writes_chainable_buffered_output() {
                ...
                cout.print("answer:").space().put(42);
    ...
    pub mod itertools {
        ...
        mod tests {
            use super::Itertools;
            #[test]
            fn collects_sorts_joins_and_deduplicates() {
                assert_eq!([3, 1, 2].into_iter().sorted().collect_vec(), [1, 2, 3]);
                assert_eq!([1, 2, 3].into_iter().join(" + "), "1 + 2 + 3");
                assert_eq!(
                    [1, 2, 1, 3, 2].into_iter().unique().collect_vec(), [1, 2, 3]
                );
                assert_eq!(
                    ["a", "bb", "c"].into_iter().unique_by(| value | value.len())
                    .collect_vec(), ["a", "bb"]
                );
            }
            #[test]
            fn counts_values_and_keys() {
                let counts = "abacaba".chars().counts();
                assert_eq!(counts[&'a'], 4);
                assert_eq!(counts[&'b'], 2);
                assert_eq!(counts[&'c'], 1);
                let parity = [1, 2, 3, 4, 5].into_iter().counts_by(|value| value % 2);
                assert_eq!(parity[& 0], 2);
                assert_eq!(parity[& 1], 3);
            }
            #[test]
            fn lazily_generates_cartesian_products() {
                assert_eq!(
                    (1..= 2).cartesian_product(['a', 'b']).collect_vec(), [(1, 'a'), (1,
                    'b'), (2, 'a'), (2, 'b')]
                );
                assert_eq!(
                    (0..2).cartesian_product(0..2).cartesian_product(0..2).count(), 8
                );
                assert_eq!((0..0).cartesian_product(0..3).next(), None);
                assert_eq!((0..3).cartesian_product(0..0).next(), None);
            }
            #[test]
            fn checks_equality() {
                assert!([7, 7, 7].into_iter().all_equal());
                assert!(! [7, 8].into_iter().all_equal());
            }
        }
    }
    pub use io::{Cin, Cout};
...
fn solve(a: String, b: String) -> Option<usize> {
    ...
    while i < a.len() && j < b.len() {
        let (i1, j1) = ((i + 1)..(a.len() + 1))
            .cartesian_product((j + 1)..(b.len() + 1))
            .find(|(i1, j1)| (pfa[*i1] - pfa[i]) % 10 == (pfb[*j1] - pfb[j]) % 10)?;
        i = i1;
        j = j1;
        k += 1;
    }
