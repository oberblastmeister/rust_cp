...
mod tests {
    use super::SegTree;
    use crate::algebra::{AddMonoid, Monoid};

    #[test]
    fn builds_queries_and_updates() {
        let mut tree: SegTree<AddMonoid<i64>> = [2_i64, 1, 4, 3, 5].into_iter().collect();

        assert_eq!(tree.len(), 5);
        assert_eq!(tree.get(2), 4);
        assert_eq!(tree[2], 4);
        assert_eq!(tree.query(1..4), 8);
        assert_eq!(tree.query(..), 15);
        assert_eq!(tree.query(..3), 7);
        assert_eq!(tree.query(2..), 12);
        assert_eq!(tree.query(1..=3), 8);
        assert_eq!(tree.query(..=2), 7);
        assert_eq!(tree.query(3..3), 0);

        tree.set(2, 10);
        assert_eq!(tree.get(2), 10);
        assert_eq!(tree[2], 10);
        assert_eq!(tree.query(1..4), 14);
    }
    ...
    #[test]
    fn preserves_order_for_non_commutative_monoids() {
        let tree: SegTree<Concat> = ["a", "b", "c", "d", "e"]
            .into_iter()
            .map(String::from)
            .collect();

        assert_eq!(tree.query(1..5), "bcde");
    }
}
